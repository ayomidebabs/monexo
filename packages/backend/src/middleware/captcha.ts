...
const captchaMiddleware = async (
  ...
  const lockoutDuration =
    parseInt(process.env.CAPTCHA_LOCKOUT_DURATION as string) || 60 * 60 * 1000;
  let captchaRecord;
  ...
  try {
    ...
    if (
      captchaRecord &&
      captchaRecord.lockedUntil &&
      captchaRecord.lockedUntil >= new Date()
    ) {
      const retryAfter = Math.ceil(
        (Number(captchaRecord.lockedUntil) - Date.now()) / 3600000
      );
      throw new Error(
        `Account locked for ${
          retryAfter > 1 ? retryAfter + ' hours' : retryAfter + ' hour'
        }`
      );
    }
  } catch (error) {
